      ...
      {expanded && (
        <div className="bg-card border-t border-border max-h-[60vh] overflow-y-auto p-4 space-y-3">
          <div className="flex items-center justify-between mb-2">
            <h3 className="font-heading font-bold text-lg">Seu Pedido</h3>
          ...
          {items.map(item => (
            <div key={item.cartId} className="flex items-center justify-between p-3 rounded-lg bg-secondary/50">
              <div className="flex-1 min-w-0">
                ...
                {item.observation && (
                  <p className="text-xs text-primary/70 mt-0.5 truncate">Obs: {item.observation}</p>
                )}
              </div>
              <div className="flex items-center gap-1 ml-2">
                ...
                <Button variant="ghost" size="icon" className="h-8 w-8 text-destructive" onClick={() => onRemoveItem(item.cartId)}>
          ...
          <div className="flex items-center justify-between pt-3 border-t border-border">
            ...
            <span className="font-heading font-bold text-xl text-primary">R$ {totalPrice.toFixed(2)}</span>
          </div>

          <Button onClick={onCheckout} className="w-full h-12 font-semibold text-base">
            Finalizar Pedido
          </Button>
        </div>
      )}
