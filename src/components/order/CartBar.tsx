...
import { ShoppingCart, ChevronUp, ChevronDown, Pencil, Trash2, Plus, Minus } from 'lucide-react';
...
export function CartBar({ items, onEditItem, onRemoveItem, onCheckout, onUpdateQuantity }: CartBarProps) {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-50 animate-slide-up">
      {expanded && (
        <div className="bg-primary border-t border-primary/80 flex flex-col max-h-[60vh]">
          {/* Sticky header with collapse arrow */}
            ...
            {items.map(item => (
              <div key={item.cartId} className="flex items-center justify-between p-3 rounded-lg bg-primary-foreground/10">
                <div className="flex-1 min-w-0">
                  ...
                  <span className="text-sm font-medium text-primary-foreground/80 mt-1 block">
                    R$ {((item.product.price + item.selectedAddons.reduce((a, ad) => a + ad.price, 0)) * item.quantity).toFixed(2)}
                  </span>
                </div>
                <div className="flex items-center gap-1 ml-2">
                  <Button variant="ghost" size="icon" className="h-8 w-8 text-primary-foreground hover:bg-primary-foreground/10" onClick={() => {
                    if (item.quantity <= 1) onRemoveItem(item.cartId);
                    else if (onUpdateQuantity) onUpdateQuantity(item.cartId, -1);
                  }}>
                    {item.quantity <= 1 ? <Trash2 className="h-3.5 w-3.5" /> : <Minus className="h-3.5 w-3.5" />}
                  </Button>
                  <span className="w-6 text-center font-bold text-sm text-primary-foreground">{item.quantity}</span>
                  <Button variant="ghost" size="icon" className="h-8 w-8 text-primary-foreground hover:bg-primary-foreground/10" onClick={() => onUpdateQuantity?.(item.cartId, 1)}>
